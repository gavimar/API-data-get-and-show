...
function addClickListeners(){
  ...
  for(let liShow of liElement){
    ...
    liShow.addEventListener('click',printSelected);
  }
}
...
function removeFav(event){
  ...
  const found = favourites.find(element => element.id === (parseInt(elemId)));
...
function printSelected(event){
  const chosen = event.currentTarget.id;
  const find = showList.find (item => item.show.id === parseInt(chosen));
  console.log(find.show.name);
}
